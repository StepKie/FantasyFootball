...
/// FlatCompetition, since the store routes through both on every save
/// and load.
/// </summary>
public class InMemoryFlatCompetitionRepositoryTests
{
	...
	public async Task RoundTrip_PreservesGameResults()
	{
		...
		var c = _definitions.Load("wm-2022");
		var g1 = (FlatGroupGame)c.Games[0];
		g1.Result = new FlatResult(2, 1, GameEnd.NORMAL);
		var g2 = (FlatGroupGame)c.Games[1];
		g2.Result = new FlatResult(0, 0, GameEnd.NORMAL);

		var id = await _repo.SaveAsync(c);
		var loaded = await _repo.GetAsync(id);

		var l1 = (FlatGroupGame)loaded!.Games[0];
		l1.Result.Should().Be(new FlatResult(2, 1, GameEnd.NORMAL));
		var l2 = (FlatGroupGame)loaded.Games[1];
		l2.Result.Should().Be(new FlatResult(0, 0, GameEnd.NORMAL));
	}
	...
	public async Task RoundTrip_PreservesResolvedKoQualifiers()
	{
		...
		var c = _definitions.Load("wm-2022");
		var ko = c.Games.OfType<FlatKoGame>().First();
		ko.HomeTeamId = "NED";
		ko.AwayTeamId = "USA";
		ko.Result = new FlatResult(3, 1, GameEnd.NORMAL);

		var id = await _repo.SaveAsync(c);
		var loaded = await _repo.GetAsync(id);

		var lko = loaded!.Games.OfType<FlatKoGame>().First(g => g.Id == ko.Id);
		lko.HomeTeamId.Should().Be("NED");
		lko.AwayTeamId.Should().Be("USA");
		lko.Result.Should().Be(new FlatResult(3, 1, GameEnd.NORMAL));
	}
