...
/// <summary>
/// Tests the spec → Competition mapping for all three bulk-sim
...
public class CompetitionFactoryTests
{
	...
	public CompetitionFactoryTests()
	{
	...
	public void Historical_PreservesDefinitionLineup()
	{
		...
		var firstGame = (GroupGame)c.Games.First(g => g is GroupGame);
	...
	public void Historical_TwoCreations_AreIndependentInstances()
	{
		...
		// sim relies on this to run N independent competitions. Uses the
		// unplayed wm-2026 so Result starts null (wm-2022 now ships baked).
		...
		((GroupGame)a.Games[0]).Result = new Result(3, 0, GameEnd.NORMAL);
		((GroupGame)b.Games[0]).Result.Should().BeNull();
	}
	...
	public void CustomLineup_ReplacesGroupAssignmentsAndGameTeams()
	{
		...
		var firstGroupAGame = c.Games.OfType<GroupGame>().First(g => g.GroupLetter == "A");
	...
	public void RandomLineup_AppliesDrawnTeamsToGroupGames()
	{
		...
		var firstGroupA = c.Games.OfType<GroupGame>().First(g => g.GroupLetter == "A");
