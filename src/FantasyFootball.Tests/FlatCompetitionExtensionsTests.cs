...
/// Hand-built FlatCompetition records — no factory, no JSON loader, no
/// simulator — just verifying the extension methods slice/dice the
/// model the way the docs claim.
/// </summary>
public class FlatCompetitionExtensionsTests
{
	static FlatCompetition BuildMiniWc(bool simulateFinal = false, bool homeWinsFinal = true)
	{
		// 2 groups × 2 teams, single round-robin in each, then a Final.
		// Tiny but exercises FlatGroupGame + FlatKoGame + Result presence/absence.
		var stages = new[]
		{
			new FlatStage("group", "Group", 0),
			new FlatStage("ko",    "Knockout", 1),
		};
		var rounds = new[]
		{
			new FlatRound("g-r1",  "Group Round 1", "group", 0),
			new FlatRound("final", "Final",         "ko",    1),
		};
		var games = new FlatGame[]
		{
			new FlatGroupGame
			{
			...
			new FlatKoGame
			{
				...
				Result = simulateFinal
					? new FlatResult { HomeScore = homeWinsFinal ? 1 : 0, AwayScore = homeWinsFinal ? 0 : 2, Ending = GameEnd.NORMAL }
					: null,
			},
		};
	...
	{
		var c = BuildMiniWc();
		var groupGame = c.Games[0];        // FlatGroupGame A
		var koGame = c.Games[2];           // FlatKoGame final
	...
	{
		var c = BuildMiniWc(simulateFinal: true);
		FlatCompetitionExtensions.HomeTeamIdOf(c.Games[0]).Should().Be("MEX");
		FlatCompetitionExtensions.HomeTeamIdOf(c.Games[2]).Should().Be("MEX");
	}

	[Fact]
	public void HomeTeamIdOf_ReturnsNullForUnresolvedKoGame()
	{
		var unresolved = new FlatKoGame
		{
			Id = 99, PlayedOn = DateTime.Now, RoundId = "final",
			HomeQual = "A1", AwayQual = "B1",
			HomeTeamId = null, AwayTeamId = null,
		};
		FlatCompetitionExtensions.HomeTeamIdOf(unresolved).Should().BeNull();
		FlatCompetitionExtensions.AwayTeamIdOf(unresolved).Should().BeNull();
	}
	...
	{
		var c = BuildMiniWc(simulateFinal: false);
		var unplayed = new FlatGroupGame
		{
	...
	public void ToString_RecordDefault_IsVerboseDebugShape()
	{
		...
		groupGame.ToString().Should().StartWith("FlatGroupGame { ")
