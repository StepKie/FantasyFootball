...
/// </summary>
public class FlatStandingsExtensionTests
{
	...
	static void SetGameResult(List<FlatGroupGame> games, string home, string away, int hs, int @as)
	{
		...
		{
			match.Result = new FlatResult(hs, @as, GameEnd.NORMAL);
		}
		else
		{
			match.Result = new FlatResult(@as, hs, GameEnd.NORMAL);
		}
	}
}
