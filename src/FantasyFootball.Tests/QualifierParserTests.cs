...
public class QualifierParserTests
{
	...
	{
		var q = QualifierParser.Parse(dsl);
		q.Should().BeOfType<GroupPlacement>();
		var gp = (GroupPlacement)q;
	...
	{
		var q = QualifierParser.Parse(dsl);
		q.Should().BeOfType<GameWinner>();
		((GameWinner)q).GameId.Should().Be(expectedId);
		q.Source.Should().Be(dsl);
	}
	...
	{
		var q = QualifierParser.Parse(dsl);
		q.Should().BeOfType<GameLoser>();
		((GameLoser)q).GameId.Should().Be(expectedId);
		q.Source.Should().Be(dsl);
	}
	...
	{
		var q = QualifierParser.Parse(dsl);
		q.Should().BeOfType<ThirdPlacePool>();
		((ThirdPlacePool)q).EligibleGroups.Should().BeEquivalentTo(expectedLetters);
		q.Source.Should().Be(dsl);
	}
	...
	{
		Action act = () => QualifierParser.Parse(dsl);
	...
	{
		QualifierParser.TryParse(dsl, out var q).Should().BeTrue();
		q!.Source.Should().Be(dsl);
	}
	...
	{
		QualifierParser.TryParse("garbage", out var q).Should().BeFalse();
		q.Should().BeNull();
	}
	...
	{
		var a = QualifierParser.Parse("A1");
		var b = QualifierParser.Parse("A1");
		a.Should().Be(b);
	...
	public void GroupL_FirstPlace_ParsesAsPlacement_NotLoser()
	{
		...
		var q = QualifierParser.Parse("L1");
		q.Should().BeOfType<GroupPlacement>();
		var gp = (GroupPlacement)q;
	...
	{
		var q = QualifierParser.Parse("L-1");
		q.Should().BeOfType<GameLoser>();
		((GameLoser)q).GameId.Should().Be(1);
	}
}
