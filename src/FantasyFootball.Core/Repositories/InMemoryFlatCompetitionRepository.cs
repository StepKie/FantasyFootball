...
/// <see cref="FlatCompetitionDefinitionLoader.Load"/> on get — same JSON
...
/// </summary>
public sealed class InMemoryFlatCompetitionRepository : IFlatCompetitionRepository
{
	readonly Dictionary<int, string> _storage = [];
	int _nextId = 1;

	public Task<int> SaveAsync(FlatCompetition competition)
	{
		if (competition.Id == 0)
		{
	...
	public Task<FlatCompetition?> GetAsync(int id)
	{
		if (!_storage.TryGetValue(id, out var json))
		{
			return Task.FromResult<FlatCompetition?>(null);
		}
		var c = FlatCompetitionDefinitionLoader.Load(json);
		c.Id = id;
		return Task.FromResult<FlatCompetition?>(c);
	}

	public Task<IReadOnlyList<FlatCompetition>> GetAllAsync()
	{
		var list = new List<FlatCompetition>(_storage.Count);
		...
		return Task.FromResult<IReadOnlyList<FlatCompetition>>(list);
	}
