...
/// <see cref="FlatCompetition"/> ready for simulation. Centralizes the
/// "spec → competition" mapping so the bulk-sim runner is a plain loop
/// and the three modes' branching logic lives in one place.
///
/// Each call yields an independent FlatCompetition — no shared mutable
/// state. The factory is safe to reuse across many specs.
///
/// <c>Flat</c> prefix is transient; the cleanup PR drops it across the
/// whole new model, leaving this as plain <c>CompetitionFactory</c>.
/// </summary>
public sealed class FlatCompetitionFactory
{
	readonly ICompetitionDefinitionStore _definitions;

	public FlatCompetitionFactory(ICompetitionDefinitionStore definitions)
	{
		_definitions = definitions;
	}

	public FlatCompetition Create(CompetitionSpec spec)
	{
	...
	static void ValidateLineupShape(string[][] lineup, FlatCompetition competition)
	{
	...
	static void ApplyLineup(FlatCompetition competition, string[][] newLineup)
	{
		...
		//    array reference is init-only on FlatCompetition; each inner
		//    string[] is mutable.
		...
		// 3. Rewire each FlatGroupGame to the new teams. Records are
		...
		{
			if (competition.Games[i] is FlatGroupGame gg)
			{
