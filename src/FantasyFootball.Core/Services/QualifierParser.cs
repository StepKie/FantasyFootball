...
/// <summary>
/// Parses qualifier DSL strings into typed <see cref="Qualifier"/>
...
{
	public static Qualifier Parse(string dsl)
	{
	...
	public static bool TryParse(string dsl, out Qualifier? result)
	{
		result = null;
		...
		if (dsl.Length >= 3 && dsl[1] == '-' && (dsl[0] == 'W' || dsl[0] == 'L'))
		{
			...
			result = dsl[0] == 'W' ? new GameWinner(gameId) : new GameLoser(gameId);
			return true;
		}
		...
		{
			result = new GroupPlacement(dsl[0].ToString(), place);
			return true;
		}
	...
	static bool TryParsePool(string dsl, out Qualifier? result)
	{
		result = null;
		var parts = dsl.Split('/');
		...
		result = new ThirdPlacePool(letters);
		return true;
	}
}
