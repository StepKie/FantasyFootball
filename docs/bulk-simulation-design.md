...
- **Efficient at scale**: sim hot path is O(games), not O(games × bracket
  depth). Serialisation produces small payloads. Per-run persistence
...
records serialise via `System.Text.Json`; both `LocalStorageRepository`
...
- `Competition → Stages → Rounds → Games` + back-references on every
  ...
  JSON `$id`/`$ref`. A WC48 graph is ~500 KB serialised.
...
/// deserialisation via System.Text.Json [JsonPolymorphic] / [JsonDerivedType].
...
**A normalised template-and-results split would save ~75% storage at
...
- Normalisation is a clean retrofit when IndexedDB lands (the runtime
  API doesn't change, only the on-disk layout).

Tracked as a known future optimisation; revisit alongside IndexedDB.

**Qualifier strings** are decoded by a small parser:

| Pattern | Meaning |
|---|---|
| `A1` | Group A's 1st-place finisher |
| `A2` | Group A's 2nd-place finisher |
| `A/B/F/G/I3` | Best 3rd-place finisher among groups A, B, F, G, I (expanded WC) |
| `W49` | Winner of game with ID 49 (game ID is its position in the Games array, 1-indexed) |
| `L61` | Loser of game 61 (3rd-place match) |
...
draw algorithm** and substituted before the spec materialises into a
Competition.
...
`GroupFactory.DrawRandom()` behaviour — pulls from the global team
...
- `FlatModel_WC48_SerializedJsonUnder100KB` — finished WC48 Competition
  serialises to < 100 KB JSON (today's graph is ~500 KB).
...
- `OldVsNew_SerialisedSize_NewIsAtLeast5xSmaller` — same setup, both
...
- [ ] Single-sim flow unchanged behaviourally (regression test).
